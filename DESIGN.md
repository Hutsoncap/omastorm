...
empty pads out to 60 are not drawn. A reserved or pending slot is a dim
hollow tick and cannot be sought. Partial (the sweep in progress) is an
outlined tick. Complete frames are solid. Each tick represents one frame,
without extra gap ticks or a baseline.
